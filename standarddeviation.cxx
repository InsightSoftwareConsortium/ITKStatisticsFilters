...
int SkewnessImageFilter( std::string inputVolume , std::string outputVolume, int radius, bool bias )
{
  ...
  typename NoiseImageFilterType::Pointer std = NoiseImageFilterType::New();
  std->SetInput(reader->GetOutput());
  std->SetRadius(radius);
  std->Update();
  typedef itk::ImageFileWriter< typename NoiseImageFilterType::OutputImageType > WriterType ;
  typename WriterType::Pointer writer = WriterType::New() ;
  writer->SetFileName( outputVolume ) ;
  writer->SetInput( std->GetOutput() ) ;
